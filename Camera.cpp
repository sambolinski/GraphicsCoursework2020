...
CCamera::CCamera()
{
	...
	m_speed = 0.025f;
    m_view_type = 0; //set to zero which is third person
}
CCamera::~CCamera()
{}
...
{  
	...
	angle_y = (float) (middle_x - mouse.x) / 8000.0f;
	angle_z = (float) (middle_y - mouse.y) / 8000.0f;
...
//Added: cycles through camera views -added
void CCamera::CycleViewType() {
    if (m_view_type < 1) {
        m_view_type = 1;
    } else {
        m_view_type = 0;
